...
class Automation:
    @Base.given('scenarios are given')
    def step_impl(context):
        path_scenario = r"/data/Scenario1.csv"  # Path to fetch Scenario file
        scenario = pd.read_csv(path_scenario)  # Read Scenario.csv file
        global scenario_file_path
        ...
        global scenariosToBeExecuted
        scenariosToBeExecuted = scenario_file_path()

    @Base.when('code the runscript')
    def step_impl(context):
        global open_app

        def open_app(url):
            global driver
            driver = webdriver.Chrome(executable_path=r"C:\Users\MALU\drivers\chromedriver.exe")
            ...
            for ind in df.index:
                ...
                if df['Event'][ind] == "click":
                    ...
                    try:
                        ...
                        WebDriverWait(driver, 30).until(
                            lambda driver: driver.find_element_by_xpath(final_xpath)).click()
                ...
                # Event used for uploading the files
                    ...
                    try:
                        ...
                        time.sleep(5)
                        WebDriverWait(driver, 30).until(
                            lambda driver: driver.find_element_by_xpath(final_xpath)).send_keys(
                            file_path)
                    takeScreenshot(ind)  # Function call for taking screenshots
...
@Base.then('url will be provided')
def step_impl(context):
